...
import { Renderer, Camera, Transform, Sphere, Program, Mesh, Orbit, Triangle, AttributeMap, Texture } from "ogl";
...
const template = `<div> 
    ...
    <div class="highlightInput">
      ...
      <input type='checkbox' \${checked<=>checkState} \${change@=>toggleHighlight}  />
    </div>
...
if (!model.entity) throw new Error("peasy broken");
...
float tex(vec2 uv){
    return  texture2D(U_noise1,uv).r;
}
...
void main() {
	...
	float ratio = 1.;
	float stars1 = texture2D(U_noise2,ratio*uv+.5).r;
    float stars2 = texture2D(U_noise2,ratio*uv+.5).r;
...
const myRenderer = new Renderer({ alpha: true });
const gl = myRenderer.gl;
myRenderer.setSize(1000, 1000);
...
//load noise textures
const noiseText1 = new Texture(gl);
const noiseText2 = new Texture(gl);

const noiseImage1 = new Image();
const noiseImage2 = new Image();

noiseImage1.onload = () => {
  noiseText1.image = noiseImage1;
};
noiseImage2.onload = () => {
  noiseText2.image = noiseImage2;
};

noiseImage1.src = noise1;
noiseImage2.src = noise2;

const uniforms = {
  U_resolution: { value: [500.0, 500.0] },
  U_time: { value: 0.0 },
  U_noise1: { value: noiseText1 },
  U_noise2: { value: noiseText2 },
  U_highlighted: { value: false },
};
...
const renderLoop = (dt: number) => {
  ...
  program.uniforms.U_highlighted.value = model.checkState;
  myRenderer.render({ scene: mesh });
};
